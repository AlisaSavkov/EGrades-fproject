...
getGrades = (id) => {
  this.props.history.push("/admin-students/"+id);
}
  ...
  render() {
    ...
    const buttons = [
      ...
      { name: "Grades", action: this.getGrades, class: "btn-third" },
      { name: "Update", action: this.updateStudent, class: "btn-update" },
      { name: "Remove", action: this.deleteStudent, class: "btn-delete" }
    ];
    ...
    return (
      <div id="subjects_wrapper">
        ...
        {this.state.students && (
          <div id="main">
            <h1 className="caption1">Students</h1>
